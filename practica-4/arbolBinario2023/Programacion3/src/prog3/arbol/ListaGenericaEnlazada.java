...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	public void invertirRecursivo(ListaGenericaEnlazada<T> lista, int cant) {
		if (lista.tamanio() > 1) {
			T tempInicio = lista.inicio.getDato();
			T tempFin = lista.fin.getDato();
			lista.eliminarEn(0);
			lista.eliminarEn(lista.tamanio() - 1);
			invertirRecursivo(lista, cant - 1);
			lista.agregarInicio(tempFin);
			lista.agregarFinal(tempInicio);
		}
	}

	public void copiarALista(ListaGenericaEnlazada<T> destino) {
		destino.comenzar();
		int i;
		for (i = 0; i < this.tamanio(); i++) {
			destino.agregarFinal(this.elemento(i));
		}
	}
	...
	public ListaGenericaEnlazada<T> invertir() {
		...
		this.copiarALista(listaInvertida);
		int cant = (int) listaInvertida.tamanio() / 2;
		invertirRecursivo(listaInvertida, cant);
